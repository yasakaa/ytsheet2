...
let cpOpenFirst = 0; // この行は変更なし

function chatPaletteOn() {
  document.querySelectorAll('.float-box:not(.chat-palette)').forEach((obj) => {
    obj.classList.remove('show');
  });
  document.querySelector('.chat-palette').classList.toggle('show');

  if (cpOpenFirst === 0) {
    chatPaletteSelect('bcdice'); // BCDice を選択
  } else {
    chatPaletteSelect(paletteTool); // paletteTool の値を使用
  }
  cpOpenFirst++;
}
...
async function downloadAsJson() {
  ...
  const jsonUrl = window.URL.createObjectURL(
    new Blob([JSON.stringify(characterDataJson)], { type: 'text/json;charset=utf-8;' })
  );
...
async function downloadAsHtml() {
  const name = document.title.replace(/ - .+?$/, '') || '無題';
  const url =
    location.href.replace(/#(.+)$/, '').replace(/&mode=(.+?)(&|$)/, '') + '&mode=download';
