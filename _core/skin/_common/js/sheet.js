...
function editOn() {
  document.querySelectorAll(".float-box:not(#login-form)").forEach((obj) => {
    obj.classList.remove("show");
  });
  document.getElementById("login-form").classList.toggle("show");
}
function loglistOn() {
  document.querySelectorAll(".float-box:not(#loglist)").forEach((obj) => {
    obj.classList.remove("show");
  });
  document.getElementById("loglist").classList.toggle("show");
}
function downloadListOn() {
  document.querySelectorAll(".float-box:not(#downloadlist)").forEach((obj) => {
    obj.classList.remove("show");
  });
...
function chatPaletteSelect(tool) {
  ...
  document
    .getElementById("cp-switch-" + (tool || "ytc"))
    .classList.add("check");
}
...
window.addEventListener("load", () => {
  if (rubyCopyMode == 0) {
    document
      .querySelectorAll("ruby:has(rp:nth-of-type(3):last-child)")
      .forEach((ruby) => {
        console.log(ruby);
...
function getJsonData(targetEnvironment = "") {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open(
      "GET",
      `./?${paramId}&mode=json&target=${targetEnvironment}`,
      true,
    );
...
async function downloadAsUdonarium() {
  ...
  const characterId =
    characterDataJson.characterName ||
    characterDataJson.monsterName ||
    characterDataJson.aka ||
    "無題";
  const image = await output.getPicture(
    characterDataJson.imageURL || defaultImage,
    "image." + characterDataJson.image,
  );
  const udonariumXml = output.generateUdonariumXml(
    generateType,
    characterDataJson,
    location.href,
    image.hash,
  );
  const udonariumUrl = await generateUdonariumZipFile(
    characterDataJson.characterName || characterDataJson.aka,
    udonariumXml,
    image,
  );
...
function getCcfoliaJson() {
  return new Promise((resolve, reject) => {
    getJsonData("ccfolia").then((characterDataJson) => {
      output
        .generateCcfoliaJson(generateType, characterDataJson, location.href)
        .then(resolve, reject);
    }, reject);
  });
}
...
function getClipboardItem() {
  try {
    return new ClipboardItem({
      "text/plain": getCcfoliaJson().then(
        (json) => {
        ...
        (err) => {
          console.error(err);
          alert(
            "キャラクターシートのデータ取得に失敗しました。通信状況等をご確認ください",
          );
        },
      ),
    });
  } catch (e) {
    // FireFox は ClipboardItem が使えない（2022/07/16 v.102.0.1）
    return {
      getType: () => {
        return new Promise(
          (resolve, reject) => {
  ...
  clipboardItem.getType("text/plain").then((blob) => {
    blob.text().then((jsonText) => {
      try {
        ...
        alert(
          "クリップボードにコピーしました。ココフォリアにペーストすることでデータを取り込めます",
        );
      } catch (e) {
...
async function downloadAsCcfolia() {
  ...
  if (navigator.clipboard && navigator.clipboard.write) {
    // FireFox は navigator.clipboard.write が使えない（2022/07/16 v.102.0.1）
...
async function downloadAsText() {
  ...
  const textData =
    output[`generateCharacterTextOf${generateType}`](characterDataJson);
  const textUrl = window.URL.createObjectURL(
...
async function downloadAsJson() {
  ...
  const jsonUrl = window.URL.createObjectURL(
    new Blob([JSON.stringify(characterDataJson)], {
...
async function downloadAsHtml() {
  const name = document.title.replace(/ - .+?$/, "") || "無題";
  const url =
    location.href.replace(/#(.+)$/, "").replace(/&mode=(.+?)(&|$)/, "") +
    "&mode=download";
...
async function downloadAsFullSet() {
  ...
  zip.file(
    name + ".html",
    await JSZipUtils.getBinaryContent(url + "&mode=download"),
  );
  zip.file(
    name + ".json",
    await JSZipUtils.getBinaryContent(url + "&mode=json"),
  );
  if (document.getElementById("chatPaletteBox"))
    zip.file(
      name + "_チャットパレット.txt",
      await JSZipUtils.getBinaryContent(url + "&mode=palette"),
    );
  ...
  if (document.getElementById("downloadlist-udonarium")) {
    ...
    zip.file(
      name + "_udonarium.zip",
      await JSZipUtils.getBinaryContent(udonariumUrl),
    );
  }
  // ココフォリア
